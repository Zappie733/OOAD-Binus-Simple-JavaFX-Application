...
public class CashierViewOrders extends BaseView{
	private OrderController orderController;
	
	
	public CashierViewOrders() {
	...
	protected VBox content(Stage mysticGrillsApp) {
		mysticGrillsApp.setTitle("Mystic Grills - Cashier Orders Page");
		VBox content = new VBox();
		content.setPadding(new Insets(30));
		content.setAlignment(Pos.CENTER);
		
		//title
        Label titleLabel = new Label("Ongoing Orders");
        ...
        activeOrderTableView.setOnMouseClicked(event -> {
            if (!activeOrderTableView.getSelectionModel().isEmpty()) {
            	...
            	CashierViewDetailOrder page = new CashierViewDetailOrder();
